...
#include "nr_diff_flat_px4_cpp/px4_utils/flight_phases.hpp"
#include "nr_diff_flat_px4_cpp/transformations/adjust_yaw.hpp"
...
public:
    OffboardControlNode(
        ...
        std::string nr_profile_name = "baseline");
...
private:
    ...
    void update_nr_error_integral(const Eigen::Vector4d& ref_now);
    ...
    Eigen::Vector4d generate_ref_position(quad_trajectories_cpp::TrajectoryType type,
                                           ...
                                           std::optional<double> t_start_override = std::nullopt);
    ...
    bool feedforward_;
    std::string nr_profile_name_;
    controller::NRProfileConfig nr_profile_;
    ...
    Eigen::Vector4d nr_error_integral_ = Eigen::Vector4d::Zero();
    double compute_time_ = 0.0;

    Eigen::Matrix<double, 12, 1> x_df_ = Eigen::Matrix<double, 12, 1>::Zero();
