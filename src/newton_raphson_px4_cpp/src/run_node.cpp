...
static void print_usage() {
    std::cerr <<
        "\nUsage: ros2 run newton_raphson_px4_cpp run_node [OPTIONS]\n"
        "\n"
        "Newton-Raphson Standard Offboard Control for Quadrotor\n"
        "\n"
        "Required:\n"
        "  --platform {sim,hw}          Platform type\n"
        "  --trajectory {hover,yaw_only,circle_horz,circle_vert,\n"
        "                fig8_horz,fig8_vert,triangle,sawtooth,\n"
        "                helix,f8_contraction}\n"
        ...
        "  --ff                         Enable feedforward (only valid with --trajectory=f8_contraction)\n"
        "\n"
        "Examples:\n"
        "  # Auto-generated log filename:\n"
        "  ros2 run newton_raphson_px4_cpp run_node --platform sim --trajectory helix --double-speed --spin --log\n"
        "  # -> logs to: src/data_analysis/log_files/newton_raphson_px4_cpp/sim_nr_std_helix_2x_spin.csv\n"
        "\n"
        "  # Custom log filename:\n"
        "  ros2 run newton_raphson_px4_cpp run_node --platform sim --trajectory helix --log --log-file my_log\n"
        "  # -> logs to: src/data_analysis/log_files/newton_raphson_px4_cpp/my_log.csv\n"
        "\n"
        "  # Feedforward (f8_contraction only):\n"
        "  ros2 run newton_raphson_px4_cpp run_node --platform sim --trajectory f8_contraction --ff --log\n"
        "  # -> logs to: src/data_analysis/log_files/newton_raphson_px4_cpp/sim_nr_std_f8_contraction_ff_1x.csv\n"
        "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    std::string sep(60, '=');
    std::cout << "\n" << sep << std::endl;
    std::cout << "Newton-Raphson Standard Offboard Control Configuration (C++)" << std::endl;
    ...
    std::cout << "Feedforward:   " << (args.feedforward ? "Enabled (f8_contraction)" : "Disabled") << std::endl;
    ...
    auto node = std::make_shared<newton_raphson_px4_cpp::OffboardControlNode>(
        args.platform, args.trajectory, args.hover_mode,
        args.double_speed, args.short_variant, args.spin,
        args.log, log_file, args.flight_period, args.feedforward);
