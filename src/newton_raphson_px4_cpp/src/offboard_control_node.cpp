#include "newton_raphson_px4_cpp/offboard_control_node.hpp"
#include "newton_raphson_px4_cpp/px4_utils/core_funcs.hpp"
...
namespace ctrl = newton_raphson_px4_cpp::controller;

static const std::string BANNER = "\n============================================================\n";

// --- Feedforward helper ---------------------------------------------------
// Analytically computes flat-output feedforward (x_ff, u_ff) for f8_contraction.
//
      ...
      offboard_mode_rc_switch_on_(sim_)
{
    ...
    battery_status_sub_ = create_subscription<px4_msgs::msg::BatteryStatus>(
        ...
        qos,
        std::bind(&OffboardControlNode::battery_status_callback, this, std::placeholders::_1));
    ...
    if (logging_enabled_) {
        ...
        csv_logger_->set_const_string(COL_CONTROLLER,  "nr");
...
void OffboardControlNode::vehicle_status_callback(
    ...
    // RCLCPP_INFO(
    //     get_logger(),
    //     "VehicleStatus | nav_state=%u arming_state=%u | "
    //     "offboard=%s armed=%s land=%s",
    //     msg->nav_state,
    //     msg->arming_state,
    //     in_offboard_mode_ ? "true" : "false",
    //     armed_ ? "true" : "false",
    //     in_land_mode_ ? "true" : "false"
    // );
...
void OffboardControlNode::publish_position_setpoint(double x, double y, double z, double yaw) {
    ...
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
        "Publishing position setpoints [%.3f, %.3f, %.3f, %.3f]", x, y, z, yaw);
}
...
void OffboardControlNode::publish_rates_setpoint(double thrust, double roll, double pitch, double yaw) {
    ...
    RCLCPP_INFO_THROTTLE(get_logger(), *get_clock(), 1000,
        "Publishing rates setpoints [thrust,r,p,y]: [%.3f, %.3f, %.3f, %.3f]",
        thrust, roll, pitch, yaw);
}

void OffboardControlNode::publish_control_timer_callback() {
    if (in_land_mode_) {
        RCLCPP_INFO(get_logger(), "In land mode...");
...
void OffboardControlNode::compute_control_timer_callback() {
    ...
    ref_ = generate_ref_position(trajectory_type_);
    {
        qt::TrajContext ctx;
        ctx.sim = sim_;
        ctx.hover_mode = hover_mode_;
        ctx.spin = spin_;
        ctx.double_speed = (trajectory_type_ == qt::TrajectoryType::F8_CONTRACTION) ? false : double_speed_;
    ...
    if (feedforward_ && trajectory_type_ == qt::TrajectoryType::F8_CONTRACTION) {
        auto ff = flat_to_x_u_f8(reference_time_, sim_);
        x_ff_ = ff.x;
        u_ff_ = ff.u;
        ff_valid_ = true;
    } else {
    ...
    // Convert to normalized inputs for PX4
    double new_force = new_input_[0];
    double new_throttle_raw = platform_->get_throttle_from_force(new_force);
    double battery_compensation = 1.0 - 0.0779 * (current_voltage_ - 16.0);
    double new_throttle = new_throttle_raw * battery_compensation;

    normalized_input_ = {new_throttle, new_input_[1], new_input_[2], new_input_[3]};
}
...
void OffboardControlNode::controller() {
    ...
    if (ff_valid_) {
        // Build the T kinematic matrix: Euler-rates = T * body-rates
        // Invert T to get body-rates from the flat-output Euler-rates.
        double roll  = roll_;
        double pitch = pitch_;
        ...
        // u_ff[1:4] are Euler rates [dphi, dth, dpsi]; convert to body rates
        ...
        // x_ff[6] is specific thrust f (m/s²); multiply by mass to get force (N)
        double thrust_ff = platform_->mass() * x_ff_[6];
        u_ff_vec << thrust_ff, body_rates_ff[0], body_rates_ff[1], body_rates_ff[2];

        // Preserve the accumulated NR correction on top of the moving
        // feedforward operating point instead of resetting back to u_ff each step.
    ...
    auto result = ctrl::newton_raphson_standard(
        nr_state_, effective_last_input, ref_,
        T_LOOKAHEAD_, LOOKAHEAD_STATE_DT_,
        compute_dt_, platform_->mass());
    new_input_ = result.u;
    cbf_term_ = result.v;
...
Eigen::Vector4d OffboardControlNode::generate_ref_position(
    qt::TrajectoryType type, std::optional<int> hover_mode_override) {

    qt::TrajContext ctx;
    ctx.sim = sim_;
    ctx.hover_mode = hover_mode_override.has_value() ? hover_mode_override : hover_mode_;
    ctx.spin = spin_;
    ctx.double_speed = (type == qt::TrajectoryType::F8_CONTRACTION) ? false : double_speed_;
    ctx.short_variant = short_variant_;

    auto fn = qt::get_trajectory_fn(type);
    return fn(reference_time_, ctx);
}
...
void OffboardControlNode::time_controller() {
    ...
    std::cout << "\n[Timing] Controller computation..." << std::endl;

    auto t0 = std::chrono::steady_clock::now();
    auto result1 = ctrl::newton_raphson_standard(
        state0, input0, ref0, T_LOOKAHEAD_, LOOKAHEAD_STATE_DT_, compute_dt_, platform_->mass());
    auto t1 = std::chrono::steady_clock::now();
    ...
    auto result2 = ctrl::newton_raphson_standard(
        state0, input0, ref0, T_LOOKAHEAD_, LOOKAHEAD_STATE_DT_, compute_dt_, platform_->mass());
    auto t3 = std::chrono::steady_clock::now();
    double time2 = std::chrono::duration<double>(t3 - t2).count();
    std::cout << "  Result (2nd call): u=[" << result2.u.transpose() << "]" << std::endl;
    std::cout << "  Time (2nd call): " << time2 << "s" << std::endl;
    std::cout << "  Good for " << (1.0 / time2) << " Hz control loop" << std::endl;
    // std::exit(0);
}

void OffboardControlNode::time_trajectories() {
    std::cout << "\n[Timing] Trajectory computation..." << std::endl;

    auto t0 = std::chrono::steady_clock::now();
    auto pos = generate_ref_position(qt::TrajectoryType::HOVER, hover_mode_);
    auto t1 = std::chrono::steady_clock::now();
    double time1 = std::chrono::duration<double>(t1 - t0).count();
    std::cout << "  Hover trajectory: [" << pos.transpose() << "], time: " << time1 << "s" << std::endl;

    auto t2 = std::chrono::steady_clock::now();
    auto pos2 = generate_ref_position(trajectory_type_);
    auto t3 = std::chrono::steady_clock::now();
    double time2 = std::chrono::duration<double>(t3 - t2).count();
    std::cout << "  Regular trajectory: [" << pos2.transpose() << "], time: " << time2 << "s" << std::endl;

    if (feedforward_ && trajectory_type_ == qt::TrajectoryType::F8_CONTRACTION) {
        auto ta = std::chrono::steady_clock::now();
        auto ff1 = flat_to_x_u_f8(0.0, sim_);
        auto tb = std::chrono::steady_clock::now();
        double ff_time1 = std::chrono::duration<double>(tb - ta).count();
        auto tc = std::chrono::steady_clock::now();
        auto ff2 = flat_to_x_u_f8(0.5, sim_);
        auto td = std::chrono::steady_clock::now();
        double ff_time2 = std::chrono::duration<double>(td - tc).count();
        (void)ff1; (void)ff2;
        std::cout << "  Feedforward (f8_contraction): time1=" << ff_time1
                  << "s, time2=" << ff_time2 << "s" << std::endl;
    }
}
