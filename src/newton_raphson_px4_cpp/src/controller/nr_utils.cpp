...
Eigen::Matrix4d get_jac_pred_u(const Eigen::Vector<double, 9>& state,
    ...
    Eigen::Vector<real, 4> u_ad = last_input.cast<real>();
    ...
    Eigen::Matrix4d J = jacobian(
        [&](Eigen::Vector<real, 4>& u) -> Eigen::Vector<real, 4> {
            return predict_output<real>(state_ad, u, T_lookahead, lookahead_step, mass);
        },
        wrt(u_ad), at(u_ad), F);
...
Eigen::Matrix4d get_inv_jac_pred_u(const Eigen::Vector<double, 9>& state,
                                    ...
                                    double lookahead_step,
                                    double mass) {
    Eigen::Matrix4d J = get_jac_pred_u(state, last_input, T_lookahead, lookahead_step, mass);
