...
- Differential-flatness feedforward for `f8_contraction` (via `--ff` flag)
...
ros2 run newton_raphson_px4_cpp run_node --platform sim --trajectory f8_contraction --double-speed --ff
...
| `--ff` | No | flag | Enable feedforward injection + add `_ff` to log filename (only valid with `f8_contraction`) |
...
When `--ff` is passed with `--trajectory=f8_contraction`, differential-flatness inversion is applied at every control step:

1. Analytically computes `x_ff = [px, py, pz, vx, vy, vz, f_specific, phi, th, psi]` and `u_ff = [df, dphi, dth, dpsi]` from the f8_contraction trajectory at the lookahead time.
...
- `sim_nr_std_f8_contraction_1x.csv`
- `sim_nr_std_f8_contraction_ff_2x.csv`
